/*!
 * jQuery CPF/CNPJ Validator Plugin v2.1.0
 *
 */
(function ($) {
    var type = null;
                ...
                if (settings.validate == 'cpf') {
                    ...
                    var opt = {
                        onKeyPress: function (data, e, field, options) {
                            ...
                            msk = (data.length > 14) ? masks[1] : masks[0];
                            ctrl.mask(msk, options);
                        }
                    };
            ...
            $(document).on(settings.event, settings.handler,
               function () {
                   if (control.val().length == 14 || control.val().length == 18) {
                       if (settings.validate == 'cpf') {
                           valid = validate_cpf(control.cleanVal());
                       }
                       else if (settings.validate == 'cnpj') {
                           valid = validate_cnpj(control.cleanVal())
                       }
                       else if (settings.validate == 'cpfcnpj') {
                           if (validate_cpf(control.cleanVal())) {
                               valid = true;
                               type = 'cpf';
                           }
                           else if (validate_cnpj(control.cleanVal())) {
                               valid = true;
                               type = 'cnpj';
                           }
                           else valid = false;
